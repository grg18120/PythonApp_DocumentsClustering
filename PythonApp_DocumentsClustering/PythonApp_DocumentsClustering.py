...
# Keep useful tokens which have Part Of Speech tag POS ['NOUN','PROPN','ADJ']
'''
usefullTokensList = [token for token in doc if token.pos_ in ['NOUN','PROPN','ADJ']]
usefullTokensGenerator = (token for token in doc if token.pos_ in ['NOUN','PROPN','ADJ'])
usefullTokensSet = {token for token in doc if token.pos_ in ['NOUN','PROPN','ADJ']}
'''
